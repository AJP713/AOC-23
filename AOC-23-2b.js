...
let dataSmall = "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green@Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue@Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red@Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red@Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green"
...
for(let i = 0; i<data[x].length; i++){
  if(data[x][i]=="b"){
    let bInd = data[x][i-4]+data[x][i-3]+data[x][i-2]
    let bVal = numberFromString(bInd)
    bVal = Number(bVal)
   // console.log(bVal)
    if(bVal > bMax){
      bMax = bVal
...
//console.log("max blue= "+bMax)

let gMax = 0
for(let i = 0; i<data[x].length; i++){
  if(data[x][i]=="g"){
    let gInd = data[x][i-4]+data[x][i-3]+data[x][i-2]
    let gVal = numberFromString(gInd)
    gVal = Number(gVal)
    //console.log(gVal)
    if(gVal > gMax){
      gMax = gVal
...
//console.log("max green= "+gMax)

let rMax = 0
for(let i = 0; i<data[x].length; i++){
  if(data[x][i]=="r"){
    let rInd = data[x][i-4]+data[x][i-3]+data[x][i-2]
    let rVal = numberFromString(rInd)
    rVal = Number(rVal)
   // console.log(rVal)
    if(rVal > rMax){
      rMax = rVal
