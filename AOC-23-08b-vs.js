...
console.log("length of directions: " + directions.length)
...
function findNextInd(curInd,dirInd){
    if(directions[dirInd]==1){
        return colStart.indexOf(colLeft[curInd])
    }else if(directions[dirInd]==2){
        return colStart.indexOf(colRight[curInd])
    }else{
        return "ERROR in findNextInd"
    }
}

function toNextZ(curInd,dirInd){
    let current = curInd
    let direct = dirInd
    let count = 0
    
    if(colStart[current][2]="Z"){
        current = findNextInd(current,direct)
        direct = (direct+1)%directions.length
        count++
    }
    while(colStart[current][2]!="Z"){
        current = findNextInd(current,direct)
        direct = (direct+1)%directions.length
        count++
    }
    console.log("z found after count: "+count)
    return [current,direct, count]
}

//tested each of the numbers, they all have a cycle exactly equal to the directions length,  if this was not the case the following code would need to be more complicated

let countToRepeat = []
let cyclesToRepeat = []
for(let i = 0; i<endA.length;i++){
    let z = toNextZ(endA[i],0)
    countToRepeat.push(z[2])
    cyclesToRepeat.push(z[2]/directions.length)
}
console.log(countToRepeat)
console.log(cyclesToRepeat)
//Hmmm they all just happen to be prime numbers of repeats so the LCM is just the product

let LCM = 1
for(let i = 0; i<cyclesToRepeat.length;i++){
    LCM = LCM * cyclesToRepeat[i]
}
LCM = LCM*directions.length
console.log(LCM)
