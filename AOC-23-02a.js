...
for(let i = 0; i<data[x].length; i++){
  ...
  if(data[x][i]=="b"){
   // console.log("b at index" + i)
    bNum = data[x][i-4]+data[x][i-3]+data[x][i-2]
    //console.log(bNum)
    let bValue = numberFromString(bNum)
    //console.log(bValue)
    if(bValue>14){
     // console.log("invalid blue at " + i)
      valid = false
    }
  }
  let rNum = ""
  if(data[x][i]=="r"){
   // console.log("b at index" + i)
    rNum = data[x][i-4]+data[x][i-3]+data[x][i-2]
    
    let rValue = numberFromString(rNum)
   // console.log(rValue)
    if(rValue>12){
      //console.log("invalid red at " + i)
      valid = false
    }
  }
  let gNum = ""
  if(data[x][i]=="g"){
    gNum = data[x][i-4]+data[x][i-3]+data[x][i-2]
    //console.log(bNum)
    let gValue = numberFromString(gNum)
    //console.log(gValue)
    if(gValue>13){
      //console.log("invalid green at " + i)
      valid = false
    }
  }
}
...
function numberFromString(string){
  ...
  if(string[0] >= "0" && string[0]<="9"){
       ...
       let threeDig =string[0]+string[1]+string[2]
    return Number(threeDig)
  }else if(string[1] >= "0" && string[1]<="9"){
    let twoDig = string[1]+string[2]
    twoDig = Number(twoDig)
    return twoDig
  }else{
    return Number(string[3])
  }
  return 0
}
